...
- **`--permission-mode` decision (, Phase 4):** the review task uses `bypassPermissions`,
 ...
 explicit denial, which would silently defeat whole premise that the agent can run
 code to verify a finding. `bypassPermissions` is the CLI's documented full-autonomy mode and is
 what actually asks for. Not independently re-verified end to end against a real,

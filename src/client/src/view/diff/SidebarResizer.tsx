 ...
 *
 * It is a `separator` with an `aria-valuenow`, not a decorative strip, because
 * a pointer-only resize is a resize half the people using the app cannot do.
 * The arrow keys move it in useful steps and Home/End take it to the stops.
 */
...
export function SidebarResizer({ width, onWidth }: SidebarResizerProps) {
	...
	const onPointerMove = useCallback(
		(event: React.PointerEvent<HTMLHRElement>) => {
			...
			// pointer's absolute position, so the handle never jumps to sit under
	...
	return (
		// an `hr`, which already means "separator", rather than a div wearing the
		// role. It is focusable here, which is what lets it carry a value and be

 ...
 * the blob endpoint enforces its own much smaller limit on top (SEC-002).
 */
...
/**
 * The raw failure of a spawned command (CON-003: adapters throw raw, never
 * AppError). Use-cases convert this into app meaning; nothing below the
 * application layer interprets it. stderr rides along as `cause` and as an
 * own field for programmatic access.
 */
...
/**
 * Thin `child_process.spawn` wrapper (ALT-002: no simple-git): argv array,
 * `shell: false`, so no interpolation ever reaches a shell. Resolves with
 * stdout decoded as utf8; rejects with ExecError on nonzero exit, timeout,
 * output overflow, or spawn failure (ENOENT propagates as-is from node).
 */

...
/**
 * The diff text for a resolved changeset, parsed into the IR (ARCHITECTURE
 * §5: "the source text is `git diff -M -C --unified=3` or the GithubService's
 * PR diff; all changeset sources go through the same parser"). Shared by
 * openReview (round r1) and refreshChangeset (round rN+1).
 */

...
describe("positional disambiguation", () => {
	...
	it("a missing branch answers with the nearest name (REQ: did you mean)", async () => {
		const { container } = buildTestContainer({
...
describe("PR resolution across backends", () => {
	...
	it("a PR the backend does not know converts to pr-not-found (CON-003)", async () => {
		const { container } = buildTestContainer({
...
describe("the announcement (REQ-008)", () => {

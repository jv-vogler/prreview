...
export function buildReviewPrompt(input: ReviewPromptInput): string {
	return [
		...
		"",
		"**The bar: an explanation earns its place only if it says something the diff does not.** The why, the cross-file cause, the consequence. Never restate the code in words. Skip mechanical changes entirely (imports, renames, fixtures, formatting); most hunks get nothing, and a run with few or no explanations is normal.",
		"",
		...
		"",
		'BAD: ["Modifies `pasteableBody` to conditionally concatenate the evidence field."] (restates the diff).',
		"",
		'GOOD: ["Evidence used to be reviewer-only scratch.", "It now ships inside the body GitHub receives.", "Every downstream edit path still touches `body` only."]',
		"",
		...
		"",
		"When several explanations serve one intent (a config change and the migration that forces it), give them the same short `topic` label so they read as one unit; leave `topic` off an explanation that stands alone.",
		"",

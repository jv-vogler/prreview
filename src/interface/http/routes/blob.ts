 ...
 * under the SEC-002 containment rules: only paths in the changeset's file
 * allowlist are servable; committed refs go through `git show <sha>:<path>`
	...
	route.get("/", async (context) => {
			...
			return context.json(
				{
					...
					message: "Blobs over 2MB are not served.",
				},
				413,
			);
		}
		if (looksBinary(content)) {
			return context.json(
				{
					reason: "blob-binary",
					message: "Binary content is not served.",
				},
				415,
			);
		}

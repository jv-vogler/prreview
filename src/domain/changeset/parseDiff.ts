...
/**
 * Wraps gitdiff-parser output into the IR of ARCHITECTURE §5: prefix chars
 * are already stripped by the parser, noEol is spread from the per-file
 * booleans onto the closing lines, rename/copy status is recovered from the
 * path pair (the parser downgrades edited renames to "modify"), and
 * function-context headers are preserved verbatim.
 */

 ...
 * deletes A and renames B → A collides on any natural key. When there is no
 * old side (no rename/copy), the path stands in for it.
 */
...
/**
 * Content-derived, position-independent hunk identity: sha256 over the
 * hunk's lines joined as `type[0] + content`. Position is deliberately
 * excluded so identity carries across rounds when content is unchanged.
 * Line content can never contain "\n", so the join is unambiguous.
 */
 ...
 * keeps the bare hash so the common case stays clean.
 */

...
export interface LaunchOptions {
	...
	/** extra environment, e.g. the `FAKE_CLAUDE_*` knobs */
	env?: Readonly<Record<string, string>>;
}

/**
 * `dist/cli.js` in a fixture repo, on a stripped PATH, with the served URL read
 * back off stdout. Every e2e spec launches its server through here so all three
 * agree on the hermetic environment: the machine's git config cannot shape what
 * the server sees, and only the fakes on `pathValue` are reachable.
 */
